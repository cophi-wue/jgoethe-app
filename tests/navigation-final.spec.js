...
const BASE_URL = "http://localhost:8765/edition.xql?c=/db/jgoethe";
...
test("navigation to Belsazar", async ({ page }) => {
  ...
  await expect(contentFrame.getByText("Belsazar")).toBeVisible({
    timeout: 15000,
  });
  ...
  await expect(
    contentFrame.locator("div.heading1").getByText("Belsazar"),
...
test("navigation to Götz von Berlichingen", async ({ page }) => {
  ...
  await expect(
    contentFrame.getByText("Götz von Berlichingen mit der eisernen Hand"),
  ).toBeVisible({ timeout: 15000 });
  await contentFrame
    .getByText("Götz von Berlichingen mit der eisernen Hand")
    .first()
    .click();
...
test("navigation to KONTEXTE - Stoffe", async ({ page }) => {
  ...
  await expect(
    contentFrame.getByText("Stoffe und Vorlagen einzelner Werke"),
  ).toBeVisible({ timeout: 15000 });
  await contentFrame
    .getByText("Stoffe und Vorlagen einzelner Werke")
    .first()
    .click();

  // Verify actual content loaded
  await expect(
    contentFrame.getByText("Paolo Rolli: La Lontananza"),
  ).toBeVisible({ timeout: 15000 });
});
...
test("full-text search for Götz", async ({ page }) => {
  ...
  await expect(hitsFrame.getByText("Treffer gefunden").first()).toBeVisible({
    timeout: 15000,
  });
  ...
  const contentFrame = page.frame({ name: "content" });
  await expect(
    contentFrame.getByText("Götz von Berlichingen").first(),
  ).toBeVisible({ timeout: 15000 });
  await expect(contentFrame.getByText("Werkkommentar")).toBeVisible();
});
